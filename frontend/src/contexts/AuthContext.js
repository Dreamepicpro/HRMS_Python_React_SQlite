...
export const AuthProvider = ({ children }) => {
  ...
  // Helper to decode JWT token without verification (for extracting username)
  ...
  const initializeAuth = async () => {
    try {
      // Clear all old non-user-specific tokens on app launch
  ...
  const login = async (credentials) => {
    try {
      ...
      // Clear any previous session revocation lock
      localStorage.removeItem('_session_revoking');
      ...
      const tokenKey = getUserStorageKey(userData.username, 'token');
      const refreshKey = getUserStorageKey(userData.username, 'refresh_token');
      const expiryKey = getUserStorageKey(userData.username, 'token_expiry');
      const userKey = getUserStorageKey(userData.username, 'user');
      const sessionKey = getUserStorageKey(userData.username, 'session_id');
      const currentUserKey = 'ves_current_user';
      ...
      // Store current logged-in username in apiClient instance (NOT in shared localStorage!)
      // This prevents cross-tab conflicts - each tab's apiClient knows its own username
      apiClient._currentUsername = userData.username;
  ...
  const logout = async () => {
    try {
      // Dispatch event BEFORE logout API call to stop all heartbeats immediately
      window.dispatchEvent(new CustomEvent('beforeLogout'));
      
      // Small delay to ensure heartbeat intervals are cleared
      ...
      const currentUsername = localStorage.getItem('ves_current_user');
      ...
      // Clear current user from localStorage (still used in some places)
      localStorage.removeItem('ves_current_user');
      
      // Clear current user from apiClient instance (most important!)
      apiClient._currentUsername = null;
      
      // Also clear any old non-user-specific keys (backward compatibility)
      localStorage.removeItem('ves_token');
      localStorage.removeItem('ves_refresh_token');
      localStorage.removeItem('ves_token_expiry');
      localStorage.removeItem('ves_user');
  ...
  // Validate current session - check if token belongs to current user
  const validateSession = useCallback(() => {
    const currentUsername = localStorage.getItem('ves_current_user');
    if (!currentUsername || !user) return true;

    // Check if stored username matches current user
    if (currentUsername !== user.username) {
      // Session conflict detected - another user logged in
      toast.error('Session conflict detected. Please login again.');
      logout();
      return false;
    }
